...
int main() {
  ...
  for (int i = 1; i < (THREADS_PER_BLOCK_X + 1); i = i << 1) {
	  hipEventRecord(start, NULL);
	  hipLaunchKernel(null_kernel,
		  dim3(TOTAL_THREADS / i, 1,1),
		  dim3(i, 1, 1),
		  0, 0,
		  deviceA);
	  ...
	  printf("block_size = [%d,%d,%d],Threads_per_cycle for Vega10 - 1.536GHz = % 3d\n", i, 1, 1, int(TOTAL_THREADS / eventMs / 1.536 / 1e6 + 0.5));
  }
